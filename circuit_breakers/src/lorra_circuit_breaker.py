...
def compute_loss(self, model, inputs, target_layers, alpha, return_outputs=False, tokenizer=None, **kwargs):
    ...
    circuit_breaker_attention_mask = inputs.get(
        f"attention_mask_circuit_breaker")
    # ==== val ====
    ...
    val_inputs = dict(input_ids=val_input_ids,
                      attention_mask=val_attention_mask, output_hidden_states=True)

    # ===== Step Coeff ====
    progress = self.get_training_progress()
    scheduled_coeff = progress
    print(f'\nPROGRESS: {progress:.4f}', '='*50)
    retain_coeff, circuit_breaker_coeff = alpha * \
        scheduled_coeff, alpha * (1-scheduled_coeff)
    ...
    with model.disable_adapter():
        ...
        with torch.no_grad():
            ...
            if log_now:
                ...
                val_hidden = torch.stack([val_outputs[l]
    ...
    if retain_coeff > 0:
        ...
        retain_loss = torch.norm(
            lora_retain_hidden - orig_retain_hidden, dim=-1, p=2, dtype=torch.float).nanmean()

        if log_now:
            retain_cosine = cosine_similarity(
                lora_retain_hidden, orig_retain_hidden, dim=-1) * layers_retain_attention_mask.squeeze(-1)
            print(
    ...
    if circuit_breaker_coeff > 0:
        ...
        normalized_lora_circuit_breaker_outputs = lora_circuit_breaker_hidden / \
            (torch.norm(lora_circuit_breaker_hidden,
             dim=-1, keepdim=True, dtype=torch.float))
        ...
        circuit_breaker_loss = torch.relu(inner_product.sum(
        ...
        if log_now:
            ...
            print("\nupdated_cb_activations_norm:",
                  updated_activations_norm.item())
            print("orig_cb_activations_norm:", orig_activations_norm.item())

            orig_cosine = cosine_similarity(
                circuit_breaker_hidden, lora_circuit_breaker_hidden, dim=-1) * layers_circuit_breaker_attention_mask.squeeze(-1)
            print(
    ...
    if log_now:
        with torch.no_grad():
            ...
            lora_val_hidden = torch.stack(
                [lora_val_outputs[l] for l in target_layers])
            layers_val_attention_mask = val_attention_mask.repeat(
                len(target_layers), 1, 1).unsqueeze(-1)

            val_cosine = cosine_similarity(
                val_hidden, lora_val_hidden, dim=-1) * layers_val_attention_mask.squeeze(-1)
            print(
...
def get_model_generation(inputs, model, tokenizer, prefill=""):
    inputs = tokenizer.apply_chat_template(
        inputs, add_generation_prompt=True, tokenize=False) + prefill
    encoded_inputs = tokenizer(inputs, return_tensors='pt')

    with torch.no_grad():
        outputs = model.generate(**encoded_inputs.to(model.device),
                                 max_new_tokens=256, do_sample=True, temperature=0.7).detach().cpu()
        sanity_generation = tokenizer.decode(
            outputs[0], skip_special_tokens=True).replace(inputs, "")
...
def data_collator(batch_list):
    ...
    for k, inputs in batch_inputs.items():
        if isinstance(inputs[0], torch.Tensor):
            ...
            raise ValueError(
                f"Return data type not implemented {type(inputs[0])}")
    return batch_inputs
...
def train():
    ...
    if "-1" in transform_layers:
        lora_layers_to_transform = [
            i for i in range(max(lorra_target_layers) + 1)]
    else:
        # transform representations
        lora_layers_to_transform = [int(layer)
                                    for layer in transform_layers.split(",")]
