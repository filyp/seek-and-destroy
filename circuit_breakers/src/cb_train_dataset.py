...
class CircuitBreakerDataset(Dataset):
    ...
    def __init__(self,
        ...
        if 'llama-3' in self.model_name_or_path:
            ...
            raise NotImplementedError(
                f"Config {self.model_name_or_path} not found")
        ...
        for example in ds:
            ...
            if switch == 0:
                formatted_input = tokenizer.apply_chat_template(
                    messages, tokenize=False).replace(tokenizer.bos_token, "")
            elif switch == 1:
        ...
        with open(f'data/xstest_v2_completions_gpt4_gpteval.csv', newline='') as f:
            ...
            data = [row for row in data if row['final_label']
                    == "1_full_compliance"]
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        combined_attention_mask_circuit_breaker = torch.cat(
            [tokenized_request_circuit_breaker["attention_mask"], response_tokenized_circuit_breaker["attention_mask"]], dim=1)

        # ========== Retain Inputs ===========
        tokenized_inputs_retain = self.tokenizer(orig_s_retain.replace(
            '<SEPARATOR>', self.sep_token), **tokenize_kwargs)

        # =========== Val Inputs ===========
        tokenized_inputs_val = self.tokenizer(val_orig.replace(
            '<SEPARATOR>', self.sep_token), **tokenize_kwargs)
