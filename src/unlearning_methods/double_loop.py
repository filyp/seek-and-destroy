...
def unlearning_func(
    ...
    unlearning_lr = trial.suggest_float("unlearning_lr", 1e-4, 1e-2, log=True)
    adv_lr = trial.suggest_float("adv_lr", 1e-4, 3e-3, log=True)
    disruption_score_decay = trial.suggest_float("disruption_score_decay", 0.5, 1)
    fork_every_n_steps = trial.suggest_int("fork_every_n_steps", 50, 300, log=True)
